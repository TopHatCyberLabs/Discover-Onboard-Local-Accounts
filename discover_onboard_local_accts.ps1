...
#
# COMMENT: 
# This script will query AD for a list of computers, then will onboard a local account (by name)
# If an account already exists in the vault, it will automatically be reconciled
# Ensure your local account platform has a reconcile account set
# Currenly only supports CyberArk Authentication
#
# WARNING:
# This is not an official CyberArk Script. Use at your own discretion
#
# VERSION HISTORY:
# 1.0 11/5/2018 - Initial release
#
###########################################################################
...
(
    ...
    [Parameter(Mandatory=$true,HelpMessage="Please enter AD Directory to query(OU=myOU,DC=mydomain,DC=com)")]
    [String]$SearchBase,
    
	[Parameter(Mandatory=$true,HelpMessage="Please enter Platform Name for accounts to vault")]
	[String]$PlatformName,
...
Function Get-LogonHeader
{
	...
	try{
	    # Logon
	    $logonToken = Invoke-RestMethod -Uri $URL_CyberArkLogon -Method "Post" -ContentType "application/json" -Body $logonBody
...
#region [Logon]
	...
	# ------------------------
	$caption = "Local Account Onboarding Utility"
...
foreach ($computer in $computers)
{
    ...
    $accExists = $(Test-Account -safeName $TargetSafe -accountName $AcctToVault -accountAddress $computer.DNSHostName)
try {
    if ($accExists -eq $false)
    {
	    # Create the Account
        $tmp_pw="1234"
        $restBody = @{account=@{username=$AcctToVault;address=$computer.DNSHostName;safe=$TargetSafe;platformID=$PlatformName;password=$tmp_pw }}| ConvertTo-Json -Depth 5
	    ...
	    {
		    Log-Msg -Type Info -MSG "Onboarded $($AcctToVault)@$($computer.DNSHostName) successfully"

            #now reconcile the password so that it's no longer known
            $n_acct = (Get-Account -accountName $AcctToVault -accountAddress $computer.DNSHostName -safeName $TargetSafe)
    ...
    Log-Msg -Type Error -MSG "There was an error onboarding $($AcctToVault)@$($computer.DNSHostName) into the Password Vault."
}
